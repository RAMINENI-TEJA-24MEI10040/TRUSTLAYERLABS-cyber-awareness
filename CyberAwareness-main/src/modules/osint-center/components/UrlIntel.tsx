...
interface UrlIntelProps {
  result: IntelligenceResult;
}

export default function UrlIntel({ result }: UrlIntelProps) {
  return (
    <div className="p-4 bg-slate-900 rounded-xl border border-cyan-500/20 shadow-sm">
      ...
      <div className="text-white mb-3">{result.summary ?? 'URL intelligence summary unavailable.'}</div>
      <div className="text-xs uppercase tracking-[0.12em] text-cyan-400 mb-3">Risk score: {result.score ?? 'N/A'}</div>
